...
var DemoList = class extends import_coc.BasicList {
  constructor(nvim) {
    super(nvim);
    this.name = "demo_list";
    this.description = "CocList for write-good";
...
async function activate(context) {
  import_coc2.window.showMessage(`write-good works!`);
  context.subscriptions.push(
    import_coc2.commands.registerCommand("write-good.Command", async () => {
      import_coc2.window.showMessage(`write-good Commands works!`);
    }),
    import_coc2.listManager.registerList(new DemoList(import_coc2.workspace.nvim)),
    import_coc2.sources.createSource({
      name: "write-good completion source",
      doComplete: async () => {
